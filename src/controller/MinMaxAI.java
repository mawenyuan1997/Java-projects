...
public abstract class MinMaxAI extends Controller {
	...
	protected @Override Location nextMove(Game g) {
		...
		for(Location loc : moves(g.getBoard())) {
			Board c = g.getBoard().update(me , loc);
			if (getScore(c , depth - 1 , me.opponent()) > maxscore) {
				maxscore = getScore(c , depth - 1 , me.opponent());
				maxloc = loc;
			} 
		}
		return maxloc;
	...
	protected int getScore(Board g , int depth , Player p) {
		if (g.getState() == State.HAS_WINNER || g.getState() == State.DRAW || depth == 0)
			return estimate(g);
		// opponent's move
		if (p == me.opponent()) {
			int minscore = Integer.MAX_VALUE;
			for(Location loc : moves(g)) {
				Board c = g.update(p , loc);
				minscore = Math.min(getScore(c , depth - 1 , p.opponent()) , minscore);
			}
			return minscore;
		}
		// minmaxAI's move
			...
			for(Location loc : moves(g)) {
				Board c = g.update(p , loc);
				maxscore = Math.max(getScore(c , depth - 1 , p.opponent()) , maxscore);
			}
			return maxscore;
		}
	}
